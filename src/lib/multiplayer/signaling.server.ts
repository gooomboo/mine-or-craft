 ...
 *
 * getSql is loaded lazily so SSR of the homepage never boots PGLite.
 */
import { z } from "zod";
...
async function roster(db: Sql, room: string): Promise<PeerRow[]> {
  const rows = await db.query<{ peer_id: string; name: string }>(
...
async function touchPeer(db: Sql, room: string, peer: string, name: string) {
  await db.query(
...
async function prune(db: Sql) {
  await Promise.all([
    db.query(`DELETE FROM webrtc_signals WHERE created_at < now() - make_interval(secs => $1)`, [
      SIGNAL_TTL_SECONDS,
    ]),
    db.query(`DELETE FROM webrtc_peers WHERE last_seen < now() - make_interval(secs => $1)`, [
      PEER_TTL_SECONDS,
    ]),
  ]);
}
...
async function handleGet(url: URL): Promise<Response> {
  ...
  if (since === 0 || Math.random() < 0.02) await prune(db);
  await touchPeer(db, room, peer, name);
...
async function handlePost(request: Request): Promise<Response> {
  ...
  if (msg.op === "signal") {
    ...
    await db.query(`DELETE FROM webrtc_peers WHERE room = $1 AND peer_id = $2`, [
      msg.room,
      msg.peer,
    ]);
  }
