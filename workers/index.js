/**
 * Cloudflare Worker: static SPA + in-memory WebRTC signaling at /api/rtc.
 * Safe for the dashboard form field:  npx wrangler deploy
 *
 * Do NOT import the rest of the game, PGLite, or Node fs.
...
export async function handleRtc(req) {
...
/** Pages Function export (functions/api/rtc.js re-exports this). */
export async function onRequest(context) {
  return handleRtc(context.request);
}

/** Workers deploy entry: /api/rtc → signaling, everything else → the game. */
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === "/api/rtc" || url.pathname.startsWith("/api/rtc/")) {
      return handleRtc(request);
    }
    if (env?.ASSETS) return env.ASSETS.fetch(request);
    return new Response("Mine or Craft — static assets missing", { status: 500 });
  },
};
